INSERT INTO agg_test.hexa_per_hostname (dt_day_utc,token,hostname, count_of_requests,sum_traffic_bytes,duration_avg_ms,duration_med_ms,q90_duration_ms,_calc_utc )

SELECT  toDate(dt_utc) as dt_day_utc ,
        replace(_token , '\0','')  as token,
        extract(_node_hostname,'(.*).hexa') as hostname,
